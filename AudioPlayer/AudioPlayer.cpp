...
void playSource(bool doEventLoop)
{
    TaskScheduler* scheduler = BasicTaskScheduler::createNew();
    UsageEnvironment* environment = BasicUsageEnvironment::createNew(*scheduler);

    SpeakerSink* sink = SpeakerSink::createNew(*environment, "fromclient.audio", true);
...
void doServer(std::string toWhom)
{
    TaskScheduler* scheduler = BasicTaskScheduler::createNew();
    BasicUsageEnvironment* env = BasicUsageEnvironment::createNew(*scheduler);

    SpeakerSink* listenSink = SpeakerSink::createNew(*env, "fromclient.audio", false);

    unsigned int rtpPortNum = 5555;
    unsigned int rtcpPortNum = rtpPortNum + 1;
    char* ipAddress = "0.0.0.0";
    //char* ipAddress = "239.255.42.42"; 
    ...
    struct in_addr address;
    address.S_un.S_addr = our_inet_addr(ipAddress);
    const Port listenRtpPort(rtpPortNum);
    const Port listenRtcpPort(rtcpPortNum);

    Groupsock2 rtpGroupSock(*env, address, listenRtpPort, 1);
    Groupsock2 rtcpGroupSock(*env, address, listenRtcpPort, 1);
    ...
    int fNumChannels = 1;
    RTPSource* rtpSource = SimpleRTPSource::createNew(
        *env, (Groupsock*)&rtpGroupSock, payloadFormatCode,
        fSamplingFrequency, "audio/L16", 0, False /*no 'M' bit*/);
    ...
    RTCPInstance* rtcpInstance =
        RTCPInstance::createNew(*env, (Groupsock*)&rtcpGroupSock, 5000, CNAME, NULL, rtpSource);

    *env << "Beginning receiving multicast stream...\n";

    listenSink->startPlaying(*rtpSource, afterPlaying, NULL);
    ...
    destinationAddress.s_addr = our_inet_addr(toWhom.c_str()); // lucas
